...
Output byte caps (`docker.output_cap_bytes`) are enforced for Docker runs only. The Sol repair scope called for extending capping to local runs as well (so a local run's transcript can't grow unaudited past the same limit); that part was not implemented — `LocalWorktreeRunner.Launch` is a direct passthrough with no capping writer. See [docs/security.md](security.md), High 11, for the honest status of this item.
