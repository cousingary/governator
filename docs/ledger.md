...
| `run_stages` | Durable stage checkpoints for one run (`PARSED`, `PREFLIGHTED`, `ROUTED`, `QUOTA_RESERVED`, `WORKSPACE_READY`, `AGENT_RUNNING`, `AUDITED`, `VALIDATING`, `ASSAYING`, `MERGE_INTENT`, `MERGE_APPLIED`, `ROOT_COMMITTED`, `MERGED`, `LEDGER_FINALIZING`, plus the terminal `APPROVED`/`QUARANTINED`/`ROLLED_BACK`/`ABANDONED`/`COMPLETE` and the conditional `OUTPUT_TRUNCATED`), one row per `(run_id, stage)`. `AGENT_RUNNING`'s detail carries the pre-launch worktree digest recovery compares against. `OUTPUT_TRUNCATED` (Session 3a; extended to local runs closing Sol High 11) is emitted when a run's transcript hit its output cap — Docker's `docker.output_cap_bytes` or local's `local.output_cap_bytes`; its detail is `accepted=<n> discarded=<n>` and it never appears silently — under the matching `require_complete_transcript` flag the run is also quarantined. `MERGE_INTENT` through `COMPLETE` are the Sol repair (Session 2) merge-transaction stages — see "Merge transaction and workspace lifecycle" below. See "Run recovery" below. |
...
| `maintenance_outbox` | Session 4: durable retry queue paired with `operational_errors` — `run_id`, `op_kind`, op-kind-specific `payload` JSON, `status` (`pending`/`done`/`dead`), `attempts`, `last_error`, `created_at`, `updated_at`. `gov reconcile` drains `pending` rows; `gov cleanup --stale` marks unrecoverable ones `dead` without deleting them. Op kinds include `quota_release` (a failed deferred reservation release — reconcile returns the headroom before the TTL would) and `workspace_destroy`, whose container-removal step tolerates only the already-gone case: any other `docker rm` failure keeps the row pending rather than marking a teardown done while the container may still be alive. |
...
## Merge transaction and workspace lifecycle

A live-root merge is not a single atomic Git operation — squashing an approved worktree's commits onto the live branch, verifying the result, and finalizing the ledger record are separate steps that can each fail independently. Session 2 of the Sol repair made this sequence transactional, with every step durably recorded:

`MERGE_INTENT` (recorded before touching the live root; requires `git status --porcelain` on the live root to be clean, and snapshots root HEAD + index state) → `MERGE_APPLIED` (squash-merge staged) → `ROOT_COMMITTED` (the live-root commit itself succeeded) → `MERGED` → `LEDGER_FINALIZING` → the terminal status stage (`APPROVED`/`QUARANTINED`) → `COMPLETE`.

**Before `ROOT_COMMITTED`**, any failure (most commonly a pre-commit hook rejecting the live-root commit) rolls the live root back: `git reset --hard <pre-merge HEAD>` plus cleanup of only the approved merge paths, restoring the root to the exact fingerprint it had before `MERGE_INTENT`. Nothing is quarantined at this point beyond the run itself — the live repository is simply untouched, verified via an empty `git status --porcelain` after rollback.

**After `ROOT_COMMITTED`**, the live-root mutation is real and must never be undone — so every remaining side effect (the `LEDGER_FINALIZING` stage write, the terminal-status stage write, `updateRun`, `RecordCompletion`, `RecordArtifacts`, and quota settlement) is wrapped: a failure there enqueues to `maintenance_outbox` and the run is recorded as `MERGED_LEDGER_PENDING` instead of surfacing as a run failure — the commit already landed, so the correct behavior is to fix the bookkeeping, not the code change. `gov reconcile` drains `MERGED_LEDGER_PENDING` rows the same way it drains any other outbox `op_kind`, replaying the terminal-status write until it lands, at which point `runs.status` self-heals to the true `APPROVED`/`QUARANTINED` value.

### Workspace cleanup guard

A crash between successful worktree creation and the run's normal completion path previously leaked the worktree directory, its registered Git worktree entry, and its `gov/job/...` branch. Immediately after `Prepare()` succeeds, a `cleanupPending` flag is armed with a deferred teardown; only normal completion (which runs the real, outcome-aware destroy) or the same in-process deferred path clears it. This closes the leak for any error returned between workspace creation and the function's normal exit, including failures from unrelated subsystems (e.g. a structural-context graph provider) that have nothing to do with the workspace itself.

### Canonical workspace lock identity

A workspace lock is keyed on the repository's *canonical* identity — `filepath.EvalSymlinks` resolution followed by device/inode identity (`syscall.Stat_t`) where available, not the raw path string — so a symlinked alias to an already-locked repository contends for the same lock instead of silently acquiring a second one. Locking itself uses an OS advisory `flock` (not an `O_EXCL` marker file), and a lock can only be released by whichever process holds a matching token recorded in the lock body, so a process that lost its lock (or a stale reclaim by a different process) can't delete a lock it doesn't actually own. Stale-lock reclaim additionally checks that the recorded PID is alive and, on Linux, that its process-start tick still matches what was recorded when the lock was taken — a recycled PID doesn't fool the reclaim check.

## Policy override identity

An override's identity (`internal/policy.overrideTarget`) is `<source layer>:<rule ID>:<rule-definition hash>`, where the rule-definition hash is a SHA-256 over the rule's full declared body (ID, `when` conditions, verdict, reason) — so an override is bound to the exact rule text that produced it, not just a rule ID that could be silently redefined underneath it. Rule-ID uniqueness is enforced per source namespace at load time (org policy, project doctrine, and job-contract-embedded rules each check independently), matching the fact that `overrideTarget` is namespaced by source.

`gov ask approve`/`deny` (`AskResolve`) reads the pending checkpoint, verifies its stored policy identity still matches, transitions its status, and inserts the resulting override row inside one SQLite transaction — a checkpoint can't be resolved twice by two racing callers (the status-transition `UPDATE` checks `RowsAffected`), and there's no window where the checkpoint shows resolved but no override exists (or vice versa). One-shot overrides (approvals/denials made without `--rule`) are claimed atomically with a guarded `UPDATE policy_overrides SET consumed_at=... WHERE id=? AND one_shot=1 AND consumed_at=''`, so two concurrent evaluations of the same job+rule can never both consume the same one-shot grant — exactly one affects a row.

`EvaluateLayers`'s `PolicyHash` is a SHA-256 over a canonical evaluation document: the evaluator version string, the final verdict, sources/reasons/consulted layers, and the full ordered per-layer result list (each carrying its own rule hash) — two decisions sharing a `PolicyHash` are provably the product of the same rules evaluating the same facts in the same order, not just a matching final verdict. Policy conditions are type-checked at load, not at evaluation time: `gt`/`gte`/`lt`/`lte` require a numeric fact and a parseable numeric value, `contains` requires a string fact and a non-empty value, `matches_any` requires a list fact and a non-empty pattern — a malformed condition (e.g. `op: gt, value: "ten"`) is a load-time error rather than a rule that silently never fires at evaluation time.
