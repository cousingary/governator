...
| `run_stages` | Durable stage checkpoints for one run (`PARSED`, `PREFLIGHTED`, `ROUTED`, `QUOTA_RESERVED`, `WORKSPACE_READY`, `AGENT_RUNNING`, `AUDITED`, `VALIDATING`, `ASSAYING`, `MERGE_INTENT`, `MERGE_APPLIED`, `ROOT_COMMITTED`, `MERGED`, `LEDGER_FINALIZING`, plus the terminal `APPROVED`/`QUARANTINED`/`ROLLED_BACK`/`ABANDONED`/`COMPLETE` and the conditional `OUTPUT_TRUNCATED`), one row per `(run_id, stage)`. `AGENT_RUNNING`'s detail carries the pre-launch worktree digest recovery compares against. `OUTPUT_TRUNCATED` (Session 3a) is emitted when a Docker run's transcript hit the output cap; its detail is `accepted=<n> discarded=<n>` and it never appears silently — under `docker.require_complete_transcript` the run is also quarantined. `MERGE_INTENT` through `COMPLETE` are the Sol repair (Session 2) merge-transaction stages — see "Merge transaction and workspace lifecycle" below. See "Run recovery" below. |
