...
5. Tag `v1.0.0` and push the tag. No release workflow runs automatically; the operator runs `scripts/release.sh` (embeds version/commit/claims-hash identity and writes the build manifest) or `goreleaser release --clean` against `.goreleaser.yml` to build artifacts.
