...
Output byte caps (`docker.output_cap_bytes`, default 20MiB) currently apply **only to Docker runs** (`internal/runner/docker.go`'s `cappedWriter`); `LocalWorktreeRunner.Launch` is a direct passthrough to the agent process with no capping writer. This is a known, undone part of the Sol repair scope (see [docs/security.md](security.md) — High 11) — a local run's transcript is not currently size-bounded the way a Docker run's is.
