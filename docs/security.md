...
**Known, accepted gap (not fixed, documented):** consumed-artifact staging (`internal/runtime/artifacts.go`'s `stageConsumedArtifacts`) seals a consumed artifact at mode `0400`, intending it read-only to the consumer. This is defeated by `--map-root-user` (the confined process runs as root inside its own user namespace, needed elsewhere for mount operations, which grants it `CAP_DAC_OVERRIDE` over files it owns) — confirmed empirically, a confined backend successfully overwrote a `0400` consumed artifact. Landlock's write ruleset has no per-subpath write-*exclusion* mechanism (only the read-only-mode write-*inclusion* carve-out above), so nothing else prevents the write either. Real fix would be a further Landlock ruleset feature; deferred by explicit decision (see the findings log) rather than fixed in this pass. `TestConsumedArtifactIsStagedReadOnlyForConsumer` now asserts only that the artifact is staged with sealed content, not that the backend is structurally unable to overwrite it.
