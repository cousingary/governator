...
| `docker.egress_allowlist` | **Rejected by validation when non-empty (fail-closed).** The docker runner has no mechanism that actually restricts egress to a `host:port` list, and an unenforced allowlist that reads as a restriction is a silently-broken security control. Use `network: deny` (the default), or an explicit `network: allow` with `sinkhole_metadata_hostnames: true`. The field is reserved for a future runner that can enforce it. |
| `docker.sinkhole_metadata_hostnames` | Optional. Under `network: allow`, redirects a fixed set of known cloud-metadata *hostnames* to loopback via `--add-host` (renamed from `deny_metadata_and_local_net` in rc3 — Sol9 P2-1 — because the old name implied it denied local networks; it never did). It does **not** block raw-IP access to metadata endpoints (e.g. `169.254.169.254`), RFC1918 networks, host gateways, local Docker services, or IPv6 link-local addresses. The only real network isolation modes are `network: deny` and `network: allow`; this field narrows the allow opt-in, it is not local-network isolation. The old key `deny_metadata_and_local_net` is still accepted for one release with a deprecation warning on stderr. |
