...
func GeneratePlan(opts Options) (contracts.Plan, error) {
	...
	for i := 1; i <= opts.Count; i++ {
		...
		jobs = append(jobs, readOnlyJob(opts.Root, id, "panel_analysis", opts.Agent, contracts.ModeArchitect, perJobTokens, []contracts.ArtifactSpec{{
	...
	comparisonID := "panel-compare"
	comparisonArtifact := "panel_comparison"
	jobs = append(jobs, readOnlyJob(opts.Root, comparisonID, "panel_comparison", opts.Agent, contracts.ModeVerifier, perJobTokens, []contracts.ArtifactSpec{{
	...
	jobs = append(jobs, readOnlyJob(opts.Root, "panel-judge", "panel_judgment", opts.Agent, contracts.ModeArchitect, perJobTokens, []contracts.ArtifactSpec{{
	...
	return contracts.Plan{Panel: &contracts.PanelSpec{ID: "panel", Members: members, ComparisonJob: comparisonID, Judge: "panel-judge"}, Jobs: jobs}, nil
}

func readOnlyJob(root, id, jobType, agent string, mode contracts.Mode, maxTokens int, produces []contracts.ArtifactSpec, consumes []string, depends []string, task string) contracts.Contract {
	...
	return contracts.Contract{
		...
		Budget:    contracts.Budget{MaxMinutes: 10, MaxCommands: 10, MaxFilesChanged: 1, MaxLinesChanged: 1, MaxNewFiles: 0, MaxDeleted: 0, MaxTokens: maxTokens},
