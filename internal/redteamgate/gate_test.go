...
// fixture) parses cleanly under the Sol12 strict decoder: exactly 221
...
// cases 209-214, and Session 4's cases 215-221, on top of upgrade-11's 195),
// plus the documented non-production exclusions that let the authoritative
// inventory account for every //go:build redteam-tagged security test
// (P0-2).
func TestLoadManifestAcceptsRealManifest(t *testing.T) {
	...
	if len(m.Cases) != 221 {
		t.Fatalf("expected 221 cases in the mandatory final attack corpus, got %d", len(m.Cases))
	}
