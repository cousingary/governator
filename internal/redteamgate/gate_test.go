...
// fixture) parses cleanly and reserves exactly 101 uniquely-named cases —
// the corpus size the current manifest mandates after rc3 Session 8's
// bundle-hygiene/claims-provenance additions (cases 96-101).
func TestLoadManifestAcceptsRealManifest(t *testing.T) {
	...
	if len(m.Cases) != 101 {
		t.Fatalf("expected 101 cases in the mandatory final attack corpus, got %d", len(m.Cases))
	}
