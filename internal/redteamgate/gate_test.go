...
// fixture) parses cleanly and reserves exactly 109 uniquely-named cases —
// the corpus size the current manifest mandates after rc4 Session 1's
// consumed-artifact-immutability additions (cases 102-109).
func TestLoadManifestAcceptsRealManifest(t *testing.T) {
	...
	if len(m.Cases) != 109 {
		t.Fatalf("expected 109 cases in the mandatory final attack corpus, got %d", len(m.Cases))
	}
