...
// fixture) parses cleanly and reserves exactly 95 uniquely-named cases —
// the corpus size the current manifest mandates after rc3 Session 7's
// symlink write-carve-out and sealed-copy-tamper additions (cases 90-95).
func TestLoadManifestAcceptsRealManifest(t *testing.T) {
	...
	if len(m.Cases) != 95 {
		t.Fatalf("expected 95 cases in the mandatory final attack corpus, got %d", len(m.Cases))
	}
