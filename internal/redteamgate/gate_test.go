...
// fixture) parses cleanly under the Sol12 strict decoder: exactly 226
...
// cases 209-214, Session 4's cases 215-221, and Session 5's cases 222-226,
// on top of upgrade-11's 195), plus the documented non-production
// exclusions that let the authoritative inventory account for every
// //go:build redteam-tagged security test (P0-2).
func TestLoadManifestAcceptsRealManifest(t *testing.T) {
	...
	if len(m.Cases) != 226 {
		t.Fatalf("expected 226 cases in the mandatory final attack corpus, got %d", len(m.Cases))
	}
