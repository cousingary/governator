...
// fixture) parses cleanly and reserves exactly 115 uniquely-named cases —
// the corpus size the current manifest mandates after rc4 Session 2's
// frozen-containment-primitives additions (cases 110-115).
func TestLoadManifestAcceptsRealManifest(t *testing.T) {
	...
	if len(m.Cases) != 115 {
		t.Fatalf("expected 115 cases in the mandatory final attack corpus, got %d", len(m.Cases))
	}
