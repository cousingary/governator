...
// fixture) parses cleanly and reserves exactly 89 uniquely-named cases —
// the corpus size the current manifest mandates after rc3 Session 6's
// Docker lifecycle-bounding additions (cases 85-89).
func TestLoadManifestAcceptsRealManifest(t *testing.T) {
	...
	if len(m.Cases) != 89 {
		t.Fatalf("expected 89 cases in the mandatory final attack corpus, got %d", len(m.Cases))
	}
