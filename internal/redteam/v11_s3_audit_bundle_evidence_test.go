...
func s3BuildCompleteDist(t *testing.T, commit string) string {
	...
	for _, name := range []string{"checksums.txt", "checksums.txt.minisig", "architecture-build-metadata.json", "sbom.json", "claims.yaml", "claims-verify-report.txt", "preflight.json", "toolset.json", "release-environment.json"} {
