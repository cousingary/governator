...
## v1.0.2 — first published release (2026-08-02)

The distribution cycle (`v16-release`, plan: `agents/governator-v1.0.2-final-release-plan.md`). `v1.0.2-rc1` through `-rc8` (upgrades 9–15) hardened the release *ceremony* on this one host without ever leaving it: no git remote, CI never run, three of four published platforms had no native evidence, `SECURITY.md` told researchers about a fixed vulnerability, and the architecture doc had grown into an 85 KB stack of release narratives. `v1.0.2` is the first release a stranger, on a different machine, can obtain, verify, and independently confirm.

- **Published, for the first time**: `github.com/cousingary/governator` and `github.com/cousingary/assayer` are now public repositories; `main` is the release branch and every release tag is reachable from it.
- **CI runs, for the first time**: `.github/workflows/ci.yml` and `release.yml` execute on GitHub-hosted runners. Real defects found and fixed rather than suppressed: Ubuntu 24.04's default AppArmor restriction on unprivileged user namespaces (`sysctl kernel.apparmor_restrict_unprivileged_userns=0`, needed by Assayer's sandboxed checks); `actions/setup-python`'s hostedtoolcache Python being group-writable, which `toolregistry` correctly refuses to trust — fixed by preferring the system `/usr/bin/python3` on Linux rather than a wrapper script (a wrapper script defeats Landlock's ELF-only read-closure computation, `internal/enforce/landlock.go`); a missing `bash` tool enrollment on the native-acceptance (`ubuntu-24.04-arm`) job; the race-tagged red-team tier's test timeout being too tight for `-race`'s instrumentation overhead on a shared runner.
- **Every published platform carries executed native acceptance evidence**: `linux_amd64`, `linux_arm64`, and `darwin_arm64` all show `approving: true` with no degraded modes. `release.yml`'s native-evidence promotion mechanism (any platform with a passing `acceptance.json`) had only ever been fed Darwin's evidence; a `linux-arm64-native-evidence` job now feeds `linux_arm64`'s too, closing a gap the first real release attempt exposed. `darwin/amd64` is intentionally not published — GitHub's free macOS runners are Apple-silicon only, so no free host could ever attest it, and Rosetta 2 runs the `arm64` archive on Intel Macs.
- **Independent verification, performed for real**: a clean-room checkout on a machine that is not this host (the VPS, not this WSL box) fetched the out-of-band trust anchor (`https://jeremylamkin.com/governator-signing-key.txt`), verified the minisign signature, checksums, and extracted-binary hash against `build-manifest.json`, and ran `gov claims verify --release` against the published bundle — all pass. Reproducibility is independently confirmed: a clean `git worktree` checkout of the `v1.0.2` tag, rebuilt with the same Go toolchain and ldflags CI used, produces a `linux_amd64` binary byte-identical to the one CI published.
- **`SECURITY.md` corrected**: the register no longer tells researchers a fixed vulnerability (High 11, closed `629cb62`) is open, and a checker (`check_release_docs.py`) now keeps it from regressing.
- **Architecture doc restructured**: split into a current-state-only record (`agents/governator_architecture.md`) and a full historical narrative (`agents/governator_architecture_history.md`), reflowed for readability, with no information lost.
- **`NOTICE`** added for the MIT-licensed ponytail ruleset adapted in `internal/minimalism`.
- Full session-by-session record: `agents/governator-v1.0.2-release-acceptance-summary.md`.

## v1.0.0 — first production release (2026-07-12)

- **`gov snap prune [--keep N]`** (default 48): removes all but the N newest snapshots, matching the legacy `harness_recall.py prune` semantics (newest-first by ID, labeled snapshots not exempt; hardlink dedup makes removing older snapshots safe for newer ones). This was the last missing piece of `docs/migration.md` step 3 — legacy snapshot cron automation can now be fully replaced by `gov snap create` + `gov snap prune`.
...
- **S3/S6 follow-up — local output capping (High 11, fully closed):** S8's documentation cross-check (below) found that High 11 — scoped into S3c and referenced again in S6's own header as "finished in S3c" — was never actually implemented for local runs. New `local.output_cap_bytes` / `local.require_complete_transcript` contract fields mirror `docker.*`: `LocalWorktreeRunner.Launch` now bounds a host subprocess's stdout/stderr through the same `cappedWriter` `DockerRunner` uses, `LocalWorktreeRunner.Observe` surfaces the accepted/discarded tally, and a capped-and-required local run is quarantined exactly like a Docker one. `runnerConfig()` (feeding `ExecutionIdentity.RunnerConfigHash`, Critical 1) now also hashes `Contract.Local` — a related gap in the same code path, found while wiring this fix, where a change to local output-cap settings could otherwise have replayed a stale approval.
